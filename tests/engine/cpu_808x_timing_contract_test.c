...
static void
test_far_returns_have_a_complete_timeline(void)
{
    const struct {
        uint8_t program[3];
        size_t size;
        ...
        { { 0xcbU, 0U, 0U }, 1U, 0x0100U, 0U, 21U, 2U },
        { { 0xcbU, 0U, 0U }, 1U, 0x0101U, 0U, 29U, 4U },
        { { 0xcaU, 0x04U, 0x00U }, 3U, 0x0100U, 4U, 24U, 2U },
        { { 0xcaU, 0x04U, 0x00U }, 3U, 0x0101U, 4U, 32U, 4U }
    };
    size_t index;
    ...
    for (index = 0U; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        ...
        bm_808x_arch_state_t state;
        uint64_t cycles = 0U;
        uint16_t stack = cases[index].stack;
        ...
        assert(state.sp ==
               (uint16_t) (stack + 4U + cases[index].adjustment));
        assert_exact_execution_clocks(&capture, cases[index].clocks);
        assert(capture.last.boundary_clock_kind ==
               BM_808X_EXECUTION_CLOCKS_EXACT);
        assert(capture.last.operand_transactions ==
...
static void
test_indirect_near_jump_has_a_complete_timeline(void)
{
    const struct {
        uint8_t program[4];
        size_t size;
        ...
        { { 0xffU, 0xe0U, 0U, 0U }, 2U, 0U, 11U, 0U },
        { { 0xffU, 0x26U, 0x00U, 0x01U }, 4U, 0x0100U, 20U, 1U },
        { { 0xffU, 0x26U, 0x01U, 0x01U }, 4U, 0x0101U, 24U, 2U }
    };
    size_t index;
    ...
    for (index = 0U; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        ...
        if (cases[index].source != 0U) {
            cpu_808x_test_poke(&machine, cases[index].source, 0x34U);
            cpu_808x_test_poke(&machine,
                               (uint16_t) (cases[index].source + 1U), 0x12U);
        }
...
static void
test_indirect_far_call_has_a_complete_timeline(void)
{
    const struct {
        uint8_t program[4];
        uint16_t pointer;
        ...
        { { 0xffU, 0x1eU, 0x00U, 0x01U },
          0x0100U, 0x0200U, 31U, 4U },
        { { 0xffU, 0x1eU, 0x01U, 0x01U },
          0x0101U, 0x0200U, 39U, 6U },
        { { 0xffU, 0x1eU, 0x00U, 0x01U },
          0x0100U, 0x0201U, 39U, 6U },
        { { 0xffU, 0x1eU, 0x01U, 0x01U },
          0x0101U, 0x0201U, 47U, 8U }
    };
    size_t index;
    ...
    for (index = 0U; index < sizeof(cases) / sizeof(cases[0]); ++index) {
        ...
        cpu_808x_test_poke(&machine,
                           (uint16_t) (cases[index].pointer + 1U), 0x12U);
        cpu_808x_test_poke(&machine,
                           (uint16_t) (cases[index].pointer + 2U), 0x00U);
        cpu_808x_test_poke(&machine,
                           (uint16_t) (cases[index].pointer + 3U), 0x20U);
