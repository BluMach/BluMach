...
| Scheduler timing | Functional approximation with measured migration coverage | One retired instruction per engine tick; the scheduler does not yet consume CPU timing observations, so rational PIT/RTC clock accumulators still use a measured functional instruction rate. Configuration v6 can forward the V30 timing observer to diagnostics. All 1,385,861 firmware boundaries now have scalar durations. The last two are documented data-dependent `F6h` unsigned byte multiplies resolved as 28 clocks by the inherited microcode's explicit no-high-half condition; this derived condition remains pending hardware confirmation. Accepted NMI and maskable INT boundaries now have scalar, alignment-aware timings too. The remaining step is wiring the CPU and timed devices to the clocked scheduler; single-step and synchronous-fault paths retain a strict unsupported guard |
