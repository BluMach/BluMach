...
11. Synthetic XTA tests exercise controller gating, switches, PIO sector read
   ...
   firmware or disk inputs. A separate clock-adapter test proves that service
   remains disarmed while idle, waits the declared interval, retries a masked
   DMA channel and returns to idle after completing the transfer.
