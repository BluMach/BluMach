...
| Scheduler timing | Functional approximation with measured migration coverage | One retired instruction per engine tick; the scheduler does not yet consume CPU timing observations, so rational PIT/RTC clock accumulators still use a measured functional instruction rate. Configuration v6 can forward the V30 timing observer to diagnostics. The firmware baseline currently yields 1,385,859 exact and 2 unknown external-boundary durations; both are documented data-dependent `F6h` unsigned byte multiplies and remain explicit pending evidence for the realised endpoint |
