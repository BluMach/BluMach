...
| Scheduler timing | Clocked runtime with strict unsupported guard | Runtime time is integer nanoseconds. The V30 reports exact boundary cycles at 10 MHz; PIT uses the exact `14.31818 MHz / 12` rational rate and the MM58167 uses its microsecond domain. All boundaries on the BIOS-only path through indexed VGA word I/O are scalar, including accepted NMI/INT and the PCS 86 immediately-ready `POLL`; unresolved ranges, busy `POLL`, single-step and synchronous faults stop instead of inventing time. XTA service remains a declared functional event every 32 microseconds pending a device deadline model. With the preserved System Disk mounted read-only, the current run passes indirect near `JMP r/m16`, advances through 6.561120 seconds of virtual time, enters the loaded boot sector at `0000:7CB1` and stops honestly at the next unplaced boundary, `REP CMPSB` |
