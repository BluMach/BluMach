...
| Scheduler timing | Clocked runtime with strict unsupported guard | Runtime time is integer nanoseconds. The V30 reports exact boundary cycles at 10 MHz; PIT uses the exact `14.31818 MHz / 12` rational rate and the MM58167 uses its microsecond domain. All 1,385,861 boundaries on the current firmware path are scalar, including accepted NMI/INT and the PCS 86 immediately-ready `POLL`; unresolved ranges, busy `POLL`, single-step and synchronous faults stop instead of inventing time. XTA service remains a declared functional event every 32 microseconds pending a device deadline model. The firmware reaches the same known `F000:85F9` endpoint and I/O count; its deterministic framebuffer CRC changes with the now-correct elapsed-time phase while CRTC state remains unchanged |
