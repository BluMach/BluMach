...
- The current PCS 86 firmware baseline completes 1,385,861 observed instruction
  ...
  Disk mounted read-only, it reaches 6.561120 seconds after placing the target
  read, prefetch suspension and queue flush of indirect near `JMP r/m16`
  (`FF /4`). Execution has entered the loaded boot sector at `0000:7CB1`; the
  next strict timing guard is `REP CMPSB`. This measured distribution sets the next
