...
such. Version 12 also routes memory and I/O transfers through the same BCU as
...
`MOV` are also placed, including odd word transfers; other operand-bearing boundaries stay unresolved until their EXU
position is known.
It is not
...
- HALT/idle, wake-up, reset and invalid-progress paths have initial tests.
  ...
  CPU migrates.
- The existing PCS 86 suite remains green. V30 observation version 12 advances
  ...
  ModR/M `MOV` now place their inherited internal waits and operand cycles on
  that timeline, and every decoded prefix
