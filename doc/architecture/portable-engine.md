...
their sum the elapsed instruction time. Timing-observation version 12 reports
...
of the four byte/word, load/store ModR/M `MOV` opcodes.
...
bus waits. Version 12 combines those quantities only for uncontended cases and
the explicitly placed `IN`/`OUT`, direct accumulator-memory `MOV` and `XLAT`
forms plus memory forms of ModR/M `MOV`; every other operand boundary remains
