 ...
 * Draft contract only. No interpreter or timing implementation exists yet.
 */
...
#define BM_286_CONTRACT_VERSION 1U
#define BM_286_STATE_VERSION 1U
...
typedef struct bm_286_boundary {
    ...
    uint64_t cpu_cycles;        /* total elapsed CPU clocks, including waits */
    uint64_t bus_wait_cycles;   /* subset of cpu_cycles, never add twice */
...
/* Conformance-only state import at a stopped boundary. Validates atomically,
 * flushes prefetch, and starts a new microarchitectural observation interval.
 * It is not a cycle-exact restore operation. */
 ...
 * Shutdown must notify the board; it must not exit the host process. */
